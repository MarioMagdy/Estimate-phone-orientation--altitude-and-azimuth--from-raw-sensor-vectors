...
def get_alt_az(reading):
    ...
    # print(H,H_no_alt_no_tilt)
    
    # G_hor = rotate_frame(tilt_angle,rotate_frame(angle_from_Y_to_Wprojection_ZY,G,axis = 0,sum_up=1,T=0),axis = 2,sum_up=0,T=0)
    # print(get_azimuth(G_hor))
    ...
    # G_no_tilt_no_alt= rotate_frame(angle_from_Y_to_Wprojection_ZY,G_no_tilt,axis = 0,sum_up=0)
    # print(get_azimuth( rotate_frame(angle_from_Y_to_Wprojection_ZY,G,axis = 0,sum_up=0,T=1)))


    print(f"real_alt_estimate: {reading['real_alt_estimate']} \t Calculated :-> {angle_from_Y_to_Wprojection_ZY}, \n real_az: {reading['real_az']}  \t Calculated :-> {get_azimuth(G_no_alt_no_tilt)},  \t Calculated :-> {get_azimuth(G_no_alt_no_sum)}, ")

    # theta_H_from_W_to_YZ =
# 


    # H_non_tilt = rotate_frame(tilt,H,axis = 2,sum_up=True)

    # new_alt,alt = calc_alt(H_non_tilt)

    # new_alt_non_tilt ,alt_non_tilt = calc_alt(H_non_tilt)

    # # G_horz_fr3 = rotate_frame(alt_non_tilt,G_non_tilt,0)
    # # print(G)

    # G_horz_fr2 = rotate_frame(alt_non_tilt,G,axis = 0)

    # G_horz_fr = rotate_frame(alt,G,axis = 0)

    # # print(get_azimuth(G_horz_fr3))
